...
def drag_coefficient_smooth_sphere_morrison(
    ...
    # Consider 0.01 to 1e7 to be safe
    if isinstance(reynolds_number, float):
        if not (reynolds_number >= 0.01 and reynolds_number <= 1e7):  # noqa: PLR2004
            warnings.warn(
                f"Reynolds number {reynolds_number} out of range {0.1, 1e7}",
                stacklevel=2,
            )
    else:
        min_re, max_re = np.min(reynolds_number), np.max(reynolds_number)
        if (min_re < 0.01) | (max_re > 1e7):  # noqa: PLR2004
            warnings.warn(
                f"Reynolds number {min_re, max_re} out of range {0.1, 1e7}",
                stacklevel=2,
            )
