...
from pytrebuchet.constants import GRAVITATIONAL_ACCELERATION_EARTH, AIR_KINEMATIC_VISCOSITY, AIR_DENSITY
from pytrebuchet.differential_equations import free_flight_ode, sling_projectile_ode, sliding_projectile_ode, ground_separation_event, projectile_release_event, projectile_hits_ground_event
...
class Simulation:
    ...
    def __init__(self, trebuchet: Trebuchet, projectile: Projectile, 
    ...
    def _solve_ground_sliding_phase(self):
        ...
        # m1, m2, 
        # ma, g
        args = (self.trebuchet.l_weight_arm, self.trebuchet.l_projectile_arm, 
                self.trebuchet.l_sling_projectile, self.trebuchet.l_sling_weight,
                self.trebuchet.d_pivot_to_arm_cog, self.trebuchet.inertia_arm, 
                self.trebuchet.mass_weight, self.projectile.mass, 
                self.trebuchet.mass_arm, self.gravitational_acceleration,
                )
        
        # Define the event to stop the integration when the projectile separates from the ground
        event = lambda t, y, *args: ground_separation_event(t, y, *args)
        event.terminal = True
        event.direction = 0

        # Define the initial conditions
        y0 = (self.trebuchet.init_angle_arm, self.trebuchet.init_angle_weight, self.trebuchet.init_angle_projectile, 
              0.0, 0.0, 0.0)
    ...
    def _solve_sling_phase(self):
        ...
        event = lambda t, y, *args: projectile_release_event(t, y, *args)
        event.terminal = True
        event.direction = 0
    ...
    def _solve_free_flight_phase(self):
        ...
        # wind_speed, rho, nu, 
        # g, projectile
        args = (self.wind_speed, self.air_density, self.air_kinematic_viscosity,
                self.gravitational_acceleration, self.projectile,
                )
        
        # Define the event to stop the integration when the projectile hits the ground
        event = lambda t, y, *args: projectile_hits_ground_event(t, y, *args)
        event.terminal = True
        event.direction = 0
       ...
       # Define the initial conditions from the end of the sling phase
        ...
        vy0 = -self.trebuchet.l_projectile_arm * dtheta0 * np.cos(theta0) + self.trebuchet.l_sling_projectile * dpsi0 * np.cos(psi0)
    ...
    def solved(self) -> bool:
        ...
        """
        return (self._solution_sliding_phase is not None and 
                self._solution_sling_phase is not None and 
