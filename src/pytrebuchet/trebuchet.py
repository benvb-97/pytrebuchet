...
class Trebuchet:
    ...
    def _calculate_initial_angles(self) -> None:
        ...
        # Arm angle
        if self.h_pivot >= self.l_projectile_arm:  # arm not long enough to reach the ground, set default angle
            self.init_angle_arm = 55. * np.pi/180.
        ...
        # Projectile angle
        _, y_arm_projectile = self.calculate_arm_endpoint_projectile(self.init_angle_arm)
        if y_arm_projectile - self.l_sling_projectile >= 0.:  # projectile does not reach the ground, sling hangs vertically downwards
            self.init_angle_projectile = -np.pi/2
        else:  # projectile just touches the ground
            self.init_angle_projectile = np.arcsin((self.l_projectile_arm * np.sin(self.init_angle_arm) - self.h_pivot) / self.l_sling_projectile)

    # Position calculation functions
    def calculate_arm_endpoint_projectile(self, angle_arm: float | np.ndarray[float]) -> tuple[float | np.ndarray[float], float | np.ndarray[float]]:
        """
        ...
        if np.any(y_arm_projectile < -1e-15):
            warnings.warn(f"Projectile arm endpoint clips through the ground. lowest y-coordinate: {np.min(y_arm_projectile)}.")

        return x_arm_projectile, y_arm_projectile

    def calculate_arm_endpoint_weight(self, angle_arm: float | np.ndarray[float]) -> tuple[float | np.ndarray[float], float | np.ndarray[float]]:
        """
        ...
        if np.any(y_arm_weight < -1e-15):
            warnings(f"Weight arm endpoint clips through the ground. lowest y-coordinate: {np.min(y_arm_weight)}.")
    ...
    def calculate_projectile_point(self,
                                  angle_arm: float | np.ndarray[float],
        ...
        x_arm_projectile, y_arm_projectile = self.calculate_arm_endpoint_projectile(angle_arm)

        x_projectile = x_arm_projectile + np.cos(angle_projectile) * self.l_sling_projectile
        y_projectile = y_arm_projectile + np.sin(angle_projectile) * self.l_sling_projectile
        
        # perform checks (projectile should not clip through the ground)
        if np.min(y_projectile < -1e15):
            warnings.warn(f"Projectile clips through the ground. lowest y-coordinate: {np.min(y_projectile)}.")
    ...
    def get_limits(self) -> tuple[tuple[float, float], tuple[float, float]]:
        ...
        max_length = projectile_length if projectile_length > weight_length else weight_length
